...
func adsCal2jd(_ iy: Int, _ im: Int, _ id: Int) -> (djm0: Double, djm: Double) {
	let IYMIN: Int = -4799 // Earliest year allowed (4800BC)
	let mtab = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
	if (iy < IYMIN) { fatalError("bad year") }
	if (im < 1 || im > 12) { fatalError("bad month") }
	let lyBool = (im == 2) && (iy%4 != 0) && ((iy%100 == 0) || (iy%400 != 0))
	let ly = lyBool ? 1 : 0
	if ((id < 1) || (id > (mtab[im-1] + ly))) {	fatalError("bad month") }
	let my: Int = (im - 14) / 12
	let iypmy = Int64(iy + my)
	let djm: Double = Double((1461 * (iypmy + 4800)) / 4 + (367 * (Int64(im) - 2 - 12 * Int64(my))) / 12 - (3 * ((iypmy + 4900) / 100)) / 4 + Int64(id) - 2432076)
	return (2400000.5, djm)
}


// Julian Date to Gregorian year, month, date, fraction
func adsJd2cal(_ dj1: Double, _ dj2: Double) -> (iy: Int, im: Int, id: Int, fd: Double) {
	let DJMIN: Double = -68569.5
	let DJMAX: Double = 1e9
	var jd, l, n, k: Int64
	var d, s, cs, x, t, f: Double
	
	let dj = dj1 + dj2
	if (dj < DJMIN || dj > DJMAX) { fatalError("unacceptable date") }
	...
	if (f - 1.0) >= -Double.ulpOfOne/4.0 {
		t = s - 1.0
		cs += (s-t) - 1.0
		s = t
		f = s + cs
		if -Double.ulpOfOne/2.0 < f {
			jd += 1
			f = adsGmax(f, 0.0)
		}
	}
	...
	l -= (1461 * i) / 4 - 31
	k = (80 * l) / 2447
	let id: Int = Int(l - (2447 * k) / 80)
	l = k / 11
	let im: Int = Int(k + 2 - 12 * l)
	let iy: Int = Int(100 * (n - 49) + i + l)
	let fd: Double = f
...
func adsJdcalf(_ ndp: Int, _ dj1: Double, _ dj2: Double) -> [Int] {
	//int js;
	//double , f1, f2, , rf;
	var status: Int = 0
	var denom, d1, d2, f, d, djd: Double
	var iymdf: [Int] = [0, 0, 0, 0]
	
	// Denominator of fraction (e.g. 100 for 2 decimal places)
	if ((ndp >= 0) && (ndp <= 9)) {
		status = 0
		denom = pow(10.0, Double(ndp))
	} else {
		status = 1
		denom = 1.0
	}
	...
	let rf = adsDnint(f * denom) / denom
	
	// Re-align to noon
	djd += 0.5
	
	// Convert to Gregorian calendar
	let (yy,mm,dd,fd) = adsJd2cal(djd, rf)
	iymdf[0] = yy
	iymdf[1] = mm
	iymdf[2] = dd
	iymdf[3] = Int(adsDnint(fd * denom))
...
func adsEpb(_ dj1: Double, _ dj2: Double) -> Double {
	let D1900 = 36524.68648 // J2000.0-B1900.0 (2415019.81352) in days
	let epb = 1900.0 + ((dj1 - adsDJ00) + (dj2 + D1900)) / adsDTY
	return epb
}


// Julian Date to Julian Epoch
func adsEpj(_ dj1: Double, _ dj2: Double) -> Double {
	let epj = 2000.0 + ((dj1 - adsDJ00) + dj2) / adsDJY
	return epj
}

// Besselian Epoch to Julian Date
func adsEpb2jd(_ epb: Double) -> (djm0: Double, djm: Double) {
	let djm = 15019.81352 + (epb - 1900.0) * adsDTY
	return (adsDJM0, djm)
}

// Julian Epoch to Julian Date
func adsEpj2jd(_ epj: Double) -> (djm0: Double, djm: Double) {
	let djm  = adsDJM00 + (epj - 2000.0) * 365.25
	return (adsDJM0, djm)
}



// Format 2-part JD for output IRAD DARE! Na, engar ok shod :) =========================
func adsD2dtf(_ scale: String, _ ndp: Int, _ d1: Double, _ d2: Double) -> (iy: Int, im: Int, id: Int, ihmsf: [Int]) {
	var leap: Int
	var iy1, im1, id1, iy2, im2, id2: Int
	var fd: Double
	var ihmsf1: [Int]
	...
	(iy1, im1, id1, fd) = adsJd2cal(a1, b1)
	
	// Is this a leap second day?
	leap = 0
	if scale == "UTC" {
		// TAI-UTC at 0h today
		let dat0 = adsDat(iy1, im1, id1, 0.0)

		// TAI-UTC at 12h today (to detect drift)
		let dat12 = adsDat(iy1, im1, id1, 0.5)
		
		// TAI-UTC at 0h tomorrow (to detect jumps)
		(iy2, im2, id2, _) = adsJd2cal(a1+1.5, b1-fd)
		let dat24 = adsDat(iy2, im2, id2, 0.0)
		
		// Any sudden change in TAI-UTC (seconds)
		let dleap = dat24 - (2.0 * dat12 - dat0)
		
		// If leap second day, scale the fraction of a day into SI
		leap = (fabs(dleap) > 0.5) ? 1 : 0
		if leap != 0 {
			fd += fd * dleap / adsDAYSEC
		}
	}
	
	// Provisional time of day
	(_, ihmsf1) = adsD2tf(ndp, fd)
...
func adsDtf2d(_ scale: String, _ iy: Int, _ im: Int, _ id: Int, _ ihr: Int, _ imn: Int, _ sec: Double) -> (d1: Double, d2: Double) {
	//Int js, iy2, im2, id2;
	//Double dj, w, seclim, dat0, dat12, dat24, dleap, time;
	var iy2, im2, id2: Int
	
	// Today's Julian Day Number
	var dj = 0.0
	var w = 0.0
	(dj, w) = adsCal2jd(iy, im, id)
	dj += w
	...
	if scale == "UTC" {
		// TAI-UTC at 0h today
		let dat0 = adsDat(iy, im, id, 0.0)

		// TAI-UTC at 12h today (to detect drift)
		let dat12 = adsDat(iy, im, id, 0.5)
		
		// TAI-UTC at 0h tomorrow (to detect jumps)
		(iy2, im2, id2, w) = adsJd2cal(dj, 1.5)
		let dat24 = adsDat(iy2, im2, id2, 0.0)
		
		// Any sudden change in TAI-UTC today and tomorrow (seconds)
		let dleap = dat24 - (2.0 * dat12 - dat0)
	...
	// The time in days
	let time = (60.0 * (Double(60 * ihr + imn)) + sec) / day
...
func adsDat(_ iy: Int, _ im: Int, _ id: Int, _ fd: Double) -> Double {
	var deltat: Double = 0.0 // TAI minus UTC, seconds
	var da: Double = 0.0
	let IYV = 2023 // Release year for this version of iauDat
	...
	let NERA1 = drift.count
	
	// Dates and Delta(AT)s
	let dat: [(Int, Int, Double)] = [
		(1960,  1,  1.4178180),
		(1961,  1,  1.4228180),
		(1961,  8,  1.3728180),
		(1962,  1,  1.8458580),
	...
	for i in dat {
		changes.append(DatRow(i))
	}
	
	// Number of Delta(AT) changes
	let NDAT = changes.count
	...
	if (fd < 0.0 || fd > 1.0) {
		fatalError("Invalid fraction of a day: \(fd). (status = -4)")
	}
	
	// Convert the date into an MJD
	let (_, djm) = adsCal2jd(iy, im, id)
	
	if iy < changes[0].iyear {
		fatalError("Pre-UTC year!")
	}
	
	if iy > IYV + 5 {
		print("Warning: suspiciously late year!")
	}
	...
	if ii < NERA1 {
		da += (djm + fd - drift[ii][0]) * drift[ii][1]
	}
...
// TDB-TT (difference between TDB and TT, for an observer on Earth)
func adsDtdb(_ date1: Double, _ date2: Double, _ ut: Double, _ elong: Double, _ u: Double, _ v: Double) -> Double {
	// 	date1,date2   Double  date, TDB
	...
	// =====================
	// 787 rows (T**0: 1-474, T**1: 475-679, T**2: 680-764, T**3: 765-784, T**4: 785-787)
	// [ amplitude (microseconds), frequency (radians per Julian millennium since J2000.0), phase (radians) ]
	//let dc: [String: [[Double]]] = Bundle.main.decode("dtdb.json")
	//let fairhd = dc["fairhd"]!
	let fairhd = [
	...
	let t = ((date1 - adsDJ00) + date2) / adsDJM
	...
	let tsol = fmod(ut, 1.0) * adsD2PI + elong
	
	// FUNDAMENTAL ARGUMENTS
	var w = t / 3600.0 // Combine time argument (millennia) with deg/arcsec factor
	let elsun = fmod(280.46645683 + 1296027711.03429 * w, 360.0) * adsDD2R // Sun Mean Longitude
	let emsun = fmod(357.52910918 + 1295965810.481 * w, 360.0) * adsDD2R   // Sun Mean Anomaly
	let d = fmod(297.85019547 + 16029616012.090 * w, 360.0) * adsDD2R      // Mean Elongation of Moon from Sun
	let elj = fmod(34.35151874 + 109306899.89453 * w, 360.0) * adsDD2R     // Mean Longitude of Jupiter
	let els = fmod(50.07744430 + 44046398.47038 * w, 360.0) * adsDD2R      // Mean Longitude of Saturn
	
	// TOPOCENTRIC TERMS:  Moyer 1981 and Murray 1983
	let wt = 0.00029e-10 * u * sin(tsol + elsun - els) + 0.00100e-10 * u * sin(tsol - 2.0 * emsun) + 0.00133e-10 * u * sin(tsol - d) + 0.00133e-10 * u * sin(tsol + elsun - elj) - 0.00229e-10 * u * sin(tsol + 2.0 * elsun + emsun) - 0.02200e-10 * v * cos(elsun + emsun) + 0.05312e-10 * u * sin(tsol - emsun) - 0.13677e-10 * u * sin(tsol + 2.0 * elsun) - 1.31840e-10 * v * cos(elsun) + 3.17679e-10 * u * sin(tsol)
	...
	let wj = 0.00065e-6 * sin(6069.776754 * t + 4.021194) + 0.00033e-6 * sin( 213.299095 * t + 5.543132) + (-0.00196e-6 * sin(6208.294251 * t + 5.696701)) + (-0.00173e-6 * sin(74.781599 * t + 2.435900)) + 0.03638e-6 * t * t 
...
// TAI to TT
func adsTaitt(_ tai1: Double, _ tai2: Double) -> (tt1: Double, tt2: Double) {
	var tt1, tt2: Double
	
	// TT minus TAI (days)
	let dtat = adsTTMTAI / adsDAYSEC
...
// TAI to UT1
func adsTaiut1(_ tai1: Double, _ tai2: Double, _ dta: Double) -> (ut11: Double, ut12: Double) {
	var ut11, ut12: Double
	
	// Result, safeguarding precision
	let dtad = dta / adsDAYSEC
	if ( fabs(tai1) > fabs(tai2) ) {
		ut11 = tai1
		ut12 = tai2 + dtad
	} else {
...
// TAI to UTC
func adsTaiutc(_ tai1: Double, _ tai2: Double) -> (utc1: Double, utc2: Double) {
	var utc1, utc2, a1, a2, u1, u2: Double
	
	// Put the two parts of the TAI into big-first order
	let big1 = ( fabs(tai1) >= fabs(tai2) )
	if big1 {
		a1 = tai1
		a2 = tai2
	} else {
	...
	for _ in 0..<3 {
		// Guessed UTC to TAI
		let (g1, g2) = adsUtctai(u1, u2)
		
		// Adjust guessed UTC
		u2 += a1 - g1  //********************************** EHTEMALAN GHALAT . Na OK ast :)
		u2 += a2 - g2
	}
...
// TCB to TDB
func adsTcbtdb(_ tcb1: Double, _ tcb2: Double) -> (tdb1: Double, tdb2: Double) {
	var tdb1, tdb2, d: Double
	
	// 1977 Jan 1 00:00:32.184 TT, as two-part JD
	let t77td = adsDJM0 + adsDJM77
	let t77tf = adsTTMTAI / adsDAYSEC
	
	// TDB (days) at TAI 1977 Jan 1.0
	let tdb0 = adsTDB0 / adsDAYSEC
	
	// Result, safeguarding precision
	if ( fabs(tcb1) > fabs(tcb2) ) {
		d = tcb1 - t77td
		tdb1 = tcb1
		tdb2 = tcb2 + tdb0 - (d + (tcb2 - t77tf)) * adsELB
	} else {
		d = tcb2 - t77td
		tdb1 = tcb1 + tdb0 - (d + (tcb1 - t77tf)) * adsELB
		tdb2 = tcb2
	}
...
// TCG to TT
func adsTcgtt(_ tcg1: Double, _ tcg2: Double) -> (tt1: Double, tt2: Double) {
	var tt1, tt2: Double
	
	// 1977 Jan 1 00:00:32.184 TT, as MJD
	let t77t = adsDJM77 + adsTTMTAI / adsDAYSEC
	
	// Result, safeguarding precision
	if ( fabs(tcg1) > fabs(tcg2) ) {
		tt1 = tcg1
		tt2 = tcg2 - ((tcg1 - adsDJM0) + (tcg2 - t77t)) * adsELG
	} else {
		tt1 = tcg1 - ((tcg2 - adsDJM0) + (tcg1 - t77t)) * adsELG
		tt2 = tcg2
	}
...
// TDB to TCB
func adsTdbtcb(_ tdb1: Double, _ tdb2: Double) -> (tcb1: Double, tcb2: Double) {
	var tcb1, tcb2, d, f: Double
...
// TDB to TT
func adsTdbtt(_ tdb1: Double, _ tdb2: Double, _ dtr: Double) -> (tt1: Double, tt2: Double) {
	var tt1, tt2: Double
	
	// Result, safeguarding precision
	let dtrd = dtr / adsDAYSEC
	if ( fabs(tdb1) > fabs(tdb2) ) {
		tt1 = tdb1
		tt2 = tdb2 - dtrd
	} else {
...
// TT to TAI
func adsTttai(_ tt1: Double, _ tt2: Double) -> (tai1: Double, tai2: Double) {
	var tai1, tai2: Double
...
// TT to TCG
func adsTttcg(_ tt1: Double, _ tt2: Double) -> (tcg1: Double, tcg2: Double) {
	var tcg1, tcg2: Double
	
	// 1977 Jan 1 00:00:32.184 TT, as MJD
	let t77t = adsDJM77 + adsTTMTAI / adsDAYSEC
	
	// TT to TCG rate
	let elgg = adsELG / (1.0 - adsELG)
	
	// Result, safeguarding precision
	if ( fabs(tt1) > fabs(tt2) ) {
		tcg1 = tt1
		tcg2 = tt2 + ((tt1 - adsDJM0) + (tt2 - t77t)) * elgg
	} else {
		tcg1 = tt1 + ((tt2 - adsDJM0) + (tt1 - t77t)) * elgg
		tcg2 = tt2
	}
...
// TT to TDB
func adsTttdb(_ tt1: Double, _ tt2: Double, _ dtr: Double) -> (tdb1: Double, tdb2: Double) {
	var tdb1, tdb2: Double
...
// TT to UT1
func adsTtut1(_ tt1: Double, _ tt2: Double, _ dt: Double) -> (ut11: Double, ut12: Double) {
	var ut11, ut12: Double
	
	// Result, safeguarding precision
	let dtd = dt / adsDAYSEC
	if ( fabs(tt1) > fabs(tt2) ) {
		ut11 = tt1
		ut12 = tt2 - dtd
	} else {
...
// UT1 to TAI
func adsUt1tai(_ ut11: Double, _ ut12: Double, _ dta: Double) -> (tai1: Double, tai2: Double) {
	var tai1, tai2: Double
...
// UT1 to TT
func adsUt1tt(_ ut11: Double, _ ut12: Double, _ dt: Double) -> (tt1: Double, tt2: Double) {
	var tt1, tt2: Double
...
// UT1 to UTC
func adsUt1utc(_ ut11: Double, _ ut12: Double, _ dut1: Double) -> (utc1: Double, utc2: Double) {
	var utc1, utc2, duts, u1, u2, d1, d2, dats1, dats2, fd, du: Double
	var iy, im, id: Int
	...
	for i in -1..<4 {
		d2 = u2 + Double(i)
		(iy, im, id, fd) = adsJd2cal(d1, d2)
		dats2 = adsDat(iy, im, id, 0.0)
		if i == -1 { 
			dats1 = dats2
		}
		...
		if fabs(ddats) >= 0.5 {
			...
			(d1, d2) = adsCal2jd(iy, im, id)
			let us1 = d1
			let us2 = d2 - 1.0 + duts / adsDAYSEC
			...
			if du > 0.0 {
				...
				fd = du * adsDAYSEC / (adsDAYSEC + ddats)
...
// UTC to TAI
func adsUtctai(_ utc1: Double, _ utc2: Double) -> (tai1: Double, tai2: Double) {
	var tai1, tai2: Double
	var u1, u2, fd, a2: Double
	var iy, im, id, iyt, imt, idt: Int
	
	// Put the two parts of the UTC into big-first order
	let big1 = ( fabs(utc1) >= fabs(utc2) )
	if big1 {
		u1 = utc1
		u2 = utc2
	} else {
	...
	(iyt, imt, idt, _) = adsJd2cal(u1+1.5, u2-fd)
	let dat24 = adsDat(iyt, imt, idt, 0.0)
	...
	fd *= (adsDAYSEC + dleap) / adsDAYSEC
	
	// Scale from (pre-1972) UTC seconds to SI seconds
	fd *= (adsDAYSEC + dlod) / adsDAYSEC
	
	// Today's calendar date to 2-part JD
	let (z1, z2) = adsCal2jd(iy, im, id)
	
	// Assemble the TAI result, preserving the UTC split and order
	a2 = z1 - u1
	a2 += z2
	a2 += fd + dat0 / adsDAYSEC
	if big1 {
		tai1 = u1
		tai2 = a2
	} else {
...
// UTC to UT1
func adsUtcut1(_ utc1: Double, _ utc2: Double, _ dut1: Double) -> (ut11: Double, ut12: Double) {
	// Look up TAI-UTC
	let (iy, im, id, _) = adsJd2cal(utc1, utc2)
	let dat = adsDat(iy, im, id, 0.0)
	...
	let (ut11, ut12) = adsTaiut1(tai1, tai2, dta)
